<template>
    <el-dialog :title="dialog.title" v-model="dialog.visible" @close="closeDialog" destroy-on-close appendToBody
        ...
        <el-form ref="formRef" :model="formData" :rules="formRules" label-width="100px">
            <el-form-item label="关联内容id" prop="item_id">
                <el-input v-model="formData.item_id" placeholder="关联内容id" size="large" clearable />
            </el-form-item>
...
const formRules = reactive({
    ...
    product_id: [{ required: true, message: "商品id为空", trigger: 'blur' }],
    img: [{ required: true, message: "商品id为空", trigger: 'blur' }],
})
/**
 * 新增/编辑成功后回调事件
 */
...
const openDialog = async (id) => {
    ...
    if (id) {
        //如果是编辑，需要查询详情回显
        dialog.title = '编辑示例';
        await instances.get(`/slideshow/slideshow/${id}`).then((res) => {
...
const submit = () => {
    formRef.value.validate(async (validate) => {
        if (validate) {
            if (formData.id) {
                //做更新操作
                instances.put("/slideshow/slideshow",formData).then((res)=>{ 
                ...
                //做新增操作
                 instances.post("/slideshow/slideshow",formData).then((res)=>{ 
