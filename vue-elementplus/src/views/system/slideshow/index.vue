<template>
    <el-card class="card"  shadow="never">
        <template #header>
            <el-form :inline="true" :model="searchForm" ref="searchFormRef">
                <el-form-item prop="product_id" label="商品id">
                    <el-input v-model="searchForm.username" placeholder="请输入账号" size="large" clearable>
                    </el-input>
                </el-form-item>
                ...
                <el-form-item>
                    ...
                    <el-button type="info" :icon="Refresh" @click="handleRest" >重置</el-button>
                </el-form-item>
            </el-form>
            <div>
        ...
        <el-table :data="pageTableData.list" border style="width: 100%" @selection-change="handleSelectionChange"   :header-cell-style="{fontSize: '13px !important' ,color:'#111  !important'}"  stripe  size="large">
            ...
            <el-table-column prop="item_id" label="关联内容id"  />
...
const loadData = async () => {
    ...
    //调用分页接口API
    await instances.post("/slideshow/slideshows/",searchForm).then(res => {
...
const openDeleteDialog = (ids) => {
    ElMessageBox.confirm(`确认删除已选中的数据项?`, '警告', {
        ...
        //调用删除接口API
        await instances.post("/slideshow/delete/",ids).then((res) => {
